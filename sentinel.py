#!/usr/bin/env python3
# SENTINEL v25.1 - Shares-based Price Filter Version
...
# Allow fractional shares (set to True if broker supports)
...
class StrategicAnalyzerV2:
    ...
    def analyze_ticker(ticker, df, sector, max_position_value_usd, vix, sec_exposures, cap_usd, market_is_bull):
        try:
            ...
            # Determine how many shares could be bought at max position value
            max_shares = int(max_position_value_usd // curr) if curr > 0 else 0
            ...
            # If after conversion no shares, skip
            if (not ALLOW_FRACTIONAL and est_shares < 1) or (ALLOW_FRACTIONAL and est_shares < 0.01):
                return None, "❌PRICE"
...
def run_mission():
    ...
    if not passed:
        ...
        for i, (ticker, r) in enumerate(passed[:MAX_NOTIFICATIONS], 1):
            ...
            if ALLOW_FRACTIONAL:
                shares_str = f"{est_shares:.4f}"
            else:
                shares_str = f"{int(est_shares)}"
