...
class CurrencyEngine:
    ...
    def get_usd_jpy():
        try:
            ...
            return round(float(df["Close"].iloc[-1]), 2) if not df.empty else 152.65
        except:
            return 152.65
...
with tab_port:
    ...
    if not pos_m:
        ...
        for s in stats_list:
            ...
            st.markdown(f'''<div class="pos-card {s['cl']}">
<div style="display: flex; justify-content: space-between; align-items: center;"><b>{s['ticker']}</b><span class="{pnl_c}">{s['pnl_pct']:+.2f}% (¥{s['pnl_usd']*fx_rate:+,.0f})</span></div>
<div style="font-size: 0.95rem; color: #f0f6fc; margin-top: 10px;">{s['shares']} shares @ ${s['avg']:.2f} (Live: ${s['cp']:.2f})</div>
<div class="exit-info">🛡️ DYNAMIC STOP: ${s['stop']:.2f}</div></div>''', unsafe_allow_html=True)
